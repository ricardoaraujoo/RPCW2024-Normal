...
g.serialize(destination="updated_ontology.ttl", format="turtle")
...
# Adicionar novas propriedades :name e :id
g.add((ns.name, RDF.type, OWL.DatatypeProperty))
g.add((ns.name, RDFS.domain, ns.Patient))
g.add((ns.name, RDFS.range, XSD.string))

g.add((ns.id, RDF.type, OWL.DatatypeProperty))
g.add((ns.id, RDFS.domain, ns.Patient))
g.add((ns.id, RDFS.range, XSD.integer))
...
with open('pg54190.json') as json_file:
    ...
    for patient_data in data:
        ...
        g.add((patient_uri, ns.name, Literal(patient_name, datatype=XSD.string)))
        g.add((patient_uri, ns.id, Literal(patient_id_counter, datatype=XSD.integer)))
